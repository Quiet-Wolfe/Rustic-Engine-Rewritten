...
pub(crate) fn scripted_stage_objects(stage_id: &str) -> Result<Vec<StageObject>> {
    match stage_id {
        ...
        // Sserafim's script-created props are registered in the stage JSON so
        // source assets, baked assets, and scene tests all see one copy.
        "sserafim" => Ok(Vec::new()),
        _ => Ok(Vec::new()),
    }
}
...
mod tests {
    ...
    fn adds_weekend_scripted_backdrops_from_stage_scripts() {
        ...
        assert!(scripted_stage_objects("sserafim").unwrap().is_empty());
